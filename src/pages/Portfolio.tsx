  ...
  return (
    ...
    >
      <div className="relative bg-white rounded-[30px_15px_30px_15px] shadow-xl hover:shadow-2xl transition-all duration-500 overflow-hidden border border-gray-100">
        ...
        {/* Floating icon */}
        <div className={`absolute -top-4 -right-4 w-12 h-12 bg-${colorClass}-600 rounded-[15px_5px_15px_5px] flex items-center justify-center transform group-hover:rotate-12 transition-transform duration-500 shadow-lg`}>
          <Icon className="h-6 w-6 text-white" />
        </div>
        
        <div className="p-8 relative z-10">
          <div className="mb-4">
            <div className={`inline-flex items-center px-3 py-1 rounded-full text-sm font-medium bg-${colorClass}-100 text-${colorClass}-700 mb-3`}>
              <Calendar className="h-4 w-4 mr-2" />
              {date}
            </div>
            
            <h3 className="text-xl font-bold text-gray-800 mb-3 leading-tight group-hover:text-gray-900 transition-colors">
              {title}
            </h3>
          </div>
          ...
          <div className="flex gap-3">
            <button className={`flex-1 flex items-center justify-center gap-2 px-4 py-3 bg-${colorClass}-600 text-white rounded-2xl hover:shadow-lg transform hover:-translate-y-0.5 transition-all duration-300`}>
              <ExternalLink className="h-4 w-4" />
              <span className="font-medium">View</span>
            </button>
            
            <button className={`px-4 py-3 border-2 border-${colorClass}-200 text-${colorClass}-600 rounded-2xl hover:bg-${colorClass}-50 transform hover:-translate-y-0.5 transition-all duration-300`}>
        ...
        <div className={`absolute bottom-0 left-0 w-20 h-20 bg-${colorClass}-200/30 rounded-tr-[40px] transform group-hover:scale-110 transition-transform duration-500`}></div>
      </div>
    </div>
  );
};
...
const Timeline = () => {
  ...
  return (
    <div className="relative">
      <h3 className="text-5xl mb-10 font-bold text-center text-gray-800 ">
        Entrepreneurial Leadership Journey
      </h3>
      
      <div className="relative max-w-4xl mx-auto">
        {/* Central timeline line */}
        <div className="absolute left-1/2 transform -translate-x-1/2 w-1 h-full bg-teal-500 rounded-full"></div>
          ...
          return (
            ...
            >
              ...
              <div className={`w-5/12 transform transition-all duration-700 ${
                ...
                <div className={`p-8 bg-white rounded-[25px_10px_25px_10px] shadow-xl hover:shadow-2xl transition-shadow duration-500 border border-gray-100 ${
                  isLeft ? 'mr-8' : 'ml-8'
                }`}>
                  <div className={`inline-flex items-center px-4 py-2 rounded-full text-sm font-medium bg-${event.color}-100 text-${event.color}-700 mb-4`}>
                    <Calendar className="h-4 w-4 mr-2" />
                    {event.date}
                  </div>
              ...
              {/* Central icon */}
              <div className={`absolute left-1/2 transform -translate-x-1/2 w-16 h-16 bg-${event.color}-500 rounded-[20px_5px_20px_5px] flex items-center justify-center shadow-lg z-10 transition-all duration-500 ${
    ...
    <div id="portfolio" className="pt-10 overflow-hidden">
      ...
      <section className="py-8 bg-slate-50 relative">
        <div className="absolute inset-0 overflow-hidden">
          ...
          <div className="absolute bottom-20 left-10 w-96 h-96 bg-teal-400/10 rounded-full blur-3xl animate-pulse delay-1000"></div>
        </div>
      ...
      <section className=" bg-white mt-5 relative">
        <div className="container mx-auto px-4">
          <div className="text-center mb-16">
            <h2 className="text-5xl font-bold text-gray-800 mb-6">Project Artifacts</h2>
          ...
          {filteredArtifacts.length === 0 && (
            <div className="text-center py-16">
              ...
              <p className="text-xl text-gray-500">No artifacts found for this filter</p>
            </div>
          )}
        </div>
      </section>
      ...
      <section className="py-10 bg-gray-50 relative overflow-hidden">
        <div className="absolute inset-0">
          <div className="absolute top-1/4 left-1/4 w-32 h-32 bg-green-200/30 rounded-full animate-pulse"></div>
          <div className="absolute bottom-1/4 right-1/4 w-20 h-20 bg-green-200/30 rounded-full animate-bounce"></div>
        </div>
